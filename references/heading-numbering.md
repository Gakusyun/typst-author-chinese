...
⚠️ 下面添加的内容均使用 Typst 原生语法编写，不依赖外部包，适用于中文 Typst 用户的排版场景。
...
⚠️ 早期 Typst 版本（如 0.14.0）对 n-1 会报错，需要使用 calc.max(n - 1, 0) 或 calc.abs(n - 1) 确保非负。
⚠️ Typst 0.14.1 及后续版本已修复该回退，通常直接使用 n-1 即可。
...
通过 show/样式规则，可以对变体标题所使用的计数器、前缀、+ 体积进行定制，从而实现视觉层面的区分。
